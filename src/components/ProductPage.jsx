...
export const ProductPage = ({ currentOrder, setCurrentOrder }) => {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            // เพิ่ม Addons ทีละตัว
            for (const a of selectedAddons) {
                await addAddonToProduct(added.s_Ord_D_Id, {
                    productId: a.addon.optionItem_Id,
                    quantity: a.qty,
                });
            }
    ...
    if (!product) {
        ...
        return (
            <div
                className="bg-bg min-h-screen p-4 text-gray-800 relative flex flex-col items-center pt-20"            >
                <div className="absolute top-1/3 flex flex-col items-center">
                    ...
                    <p className="mt-4 text-sm font-bold tracking-widest uppercase text-main">Loading Product...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="w-full text-stone-800 min-h-screen bg-bg"
        >
            <div className="relative pb-28"> {/* pb-28 เพื่อให้มีพื้นที่สำหรับ Sticky Footer */}
               ...
               focus:outline-none focus:ring-2 focus:ring-main"
                ...
                <div
                    className="fixed bottom-0 left-0 w-full z-20 bg-white shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.1)] flex justify-between items-center px-6 py-4 rounded-t-2xl"
                >
